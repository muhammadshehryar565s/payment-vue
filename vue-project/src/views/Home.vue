    ...
    <div>
      <div class="box-inner-2">
        ...
        <div class="mb-3">
          ...
          <input class="form-control" type="text" v-model="name" @input="validateInput">
          <p class="dis fw-bold mb-2">Email address</p>
          <input class="form-control" type="email" v-model="email" @input="validateInput">
          <p class="dis fw-bold mb-2">Phone Number</p>
          <input class="form-control" type="number" v-model="phone" @input="validateInput">
          <p class="dis fw-bold mb-2">Address</p>
          <input class="form-control" type="text" v-model="address" @input="validateInput">
          <p class="dis fw-bold mb-2">Total bills</p>
          <input class="form-control" type="number" v-model="bill" @input="validateInput">
        </div>
        <div>
          <h4>Card details</h4>
        ...
        <button @click="submitPayment" :disabled="!isFormValid" class="submitclass">Submit</button>
        <p v-if="!isFormValid" class="text-danger">Please fill in all required fields</p>
      </div>
    </div>
    <ConfirmationModal :visible="isModalVisible" @confirm="handleConfirm" @cancel="isModalVisible = false"/>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.stripe = Stripe('pk_test_51PMsGkAHsijzAzJEE2Lx1TDsXFJzFTQHPmwC4e5cBLPmnNjCjC47lAJJQZGQDQpjI5u15NRdhILsYEaLRgESMHld00HEctnnAQ');
  ...
  methods: {
    validateInput() {
      this.isFormValid = !!this.name && !!this.email && !!this.phone && !!this.address && !!this.bill;
    },
    showModal() {
      if (this.isFormValid) {
    ...
    processPayment(token) {
      ...
      axios.post('/courses/payment/stripe/', formData)
        .then(response => {
      ...
      axios.post('/courses/payment/', formData)
        .then(response => {
          // console.log('Backend payment process complete', response.data);
        })
        .catch(error => {
